 ...
 * 
 * 使用方法：
 * npx tsx scripts/test-taobao-api.ts
 */
...
async function runTests() {
  ...
  try {
    const connected = await testTaoBaoAPI()
    if (connected) {
      console.log('✅ 连接成功！')
    } else {
  ...
  try {
    ...
    const products = await getTaoBaoItemList({ limit: 5 })
    ...
    if (products.length > 0) {
      ...
      products.slice(0, 3).forEach((product, index) => {
        ...
        console.log(`   淘口令：${product.promoCode ? '✅' : '❌'}`)
      })
    } else {
  ...
  // 测试 3: 搜索特定商品
  console.log('【测试 3】搜索 "羽绒服"...')
  try {
    const startTime = Date.now()
    const products = await getTaoBaoItemList({ q: '羽绒服', limit: 3 })
    const duration = Date.now() - startTime
  ...
  // 测试 4: 价格区间筛选
  console.log('【测试 4】价格区间筛选 (100-500 元)...')
  try {
    const products = await getTaoBaoItemList({ 
      startPrice: 100, 
      endPrice: 500, 
      limit: 3 
    })
